...
export const getBlogPostPage = (slug: string): number => {
  const idx = blogPosts.findIndex((p) => p.slug === slug);
  if (idx < 0) return 1;
  return Math.floor(idx / POSTS_PER_PAGE) + 1;
};
...
export const getAdjacentBlogPosts = (slug: string): AdjacentBlogPosts => {
  const idx = blogPosts.findIndex((p) => p.slug === slug);
  const totalPages = Math.max(1, Math.ceil(blogPosts.length / POSTS_PER_PAGE));
  if (idx < 0) {
    return { prev: null, next: null, currentPage: 1, totalPages };
  }
  return {
    prev: idx > 0 ? blogPosts[idx - 1] : null,
    next: idx < blogPosts.length - 1 ? blogPosts[idx + 1] : null,
    currentPage: Math.floor(idx / POSTS_PER_PAGE) + 1,
    totalPages,
  };
};
